 ...
 * 
 ***********************************************************/
public class WFactura extends JanelaFactura
{
    ...
    ***********************************************************/
	...
	class L_CodigoFactura implements ActionListener
	{
		...
		if (factura_corrente == null)
		    factura_corrente = new Factura(numero, Data.hoje(), false);
	...
	class L_TipoProduto implements ActionListener
	{
		...
		{
		    if (rb_perecivel.isSelected() && p instanceof Perecivel)
			cb_produtos.addItem(p);
		    if (rb_nperecivel.isSelected() && p instanceof NaoPerecivel)
			cb_produtos.addItem(p);
		}
	    }
	}
	...
	class L_Subtotal implements ActionListener
	{
	    public void actionPerformed(ActionEvent event)
	    {
		f_subtotal.setText(Double.parseDouble(f_quanto.getText())
			* Double.parseDouble(f_valor.getText()) + "");
	    }
	}
	...
	class L_AdicionaLinha implements ActionListener
	{
		...
		if (factura_corrente.isAnulada())
		    JOptionPane.showMessageDialog(janelaFactura, "Não se pode modificar uma factura anulada!", 
	...
	class L_RetiraLinha implements ActionListener
	{
		...
		{
		    factura_corrente.retira((Produto) cb_produtos.getSelectedItem(), 
			    Double.parseDouble(f_quanto.getText()));
		    escreve(factura_corrente);
		}
	    }
	...
	class L_ApagaFactura implements ActionListener
	{
	    public void actionPerformed(ActionEvent event)
	    {
		if (JOptionPane.showConfirmDialog(janelaFactura,
			"Apagar factura " + factura_corrente + " da base de dados?",
			"Confirmar", JOptionPane.YES_NO_OPTION,
			JOptionPane.WARNING_MESSAGE) == 0)
		    getBDados().retira(factura_corrente);
	...
	class L_AnulaFactura implements ActionListener
	{
	    ...
	    {
		if (JOptionPane.showConfirmDialog(janelaFactura,
			"Anular factura " + factura_corrente + "?", "Confirmar",
	...
	class L_FechaFactura implements ActionListener
	{
		...
		JOptionPane.showMessageDialog(janelaFactura,
			"Inserida factura " + factura_corrente + " na base de dados!",
